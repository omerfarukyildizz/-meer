...
namespace Pbk.Core.Features.Carriers.Create
{
    ...
    public sealed class CarrierCreateCommandValidator : AbstractValidator<CarrierCreateCommand>
    {
        public CarrierCreateCommandValidator()
        {
            RuleFor(x => x.DepartmentId)
                .NotEmpty().WithMessage("DepartmanId alanı boş olamaz.");
               
            RuleFor(x => x.TimocomId)
                .NotEmpty().WithMessage("TimocomId alanı boş olamaz.");
